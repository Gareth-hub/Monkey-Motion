...
os.chdir("D:\Programming\Monkey Motion\Monkey Motion Game")		# Changing directory for correct pathing on files
...
xspeed = 0				# Variables created for speed and accelleration of monkey
raccel = 0
laccel = 0
...
for i in range(12):
    enemy_copy = Block("images/unhero2.png")
    ...
    '''
    block.center_x = random.randrange(1400)
    block.center_y = random.randrange(650)
    '''
...
while True:
    ...
    banana_hit=pygame.sprite.spritecollide(monkey, banana_list, True)
    #if pygame.sprite.collide_rect(monkey, banana_list):
    #    ++banana_count
    #    print (banana_count)
    banana_list.update()

    enemy_hit=pygame.sprite.spritecollide(monkey, enemy_list, True)
    enemy_list.update()
    ...
    xspeed -= 0.1 + raccel      # add a bit of acceleration due to gravity
    xspeed += 0.1 - laccel
    tick(60)
